...
The entry evaluator in this document covers the new-trade funnel only. Open-position exits are handled by the separate `intraday_risk_monitor_stop30m_v004` model in `artifacts/risk_model_registry.json`, currently configured at `threshold = 0.08`, `confirmations_required = 2`, and `min_age_minutes = 10`.
